...
def infer(config: InferenceConfig):
    ...
    if config.toploc.enable_toploc2:
        llm.llm_engine.model_executor.driver_worker.model_runner.sampler = (
            Toploc2Sampler()
        )
    ...
    # Initialize dataset
    logger.info(
        f"Initializing dataset (name={config.data.name}, split={config.data.split})"
    )
    start_time = time.time()
    ...
    logger.success(
        f"Initialized dataset with {len(dataset):,} problems in {time.time() - start_time:.2f}s"
    )
    ...
    if config.group_id is not None:
        ...
        seed = (
            config.seed + int(os.environ.get("DP_RANK", 0))
    ...
    if config.data.max_prompt_len:
        logger.info(
            f"Filtering out prompts with more than {config.data.max_prompt_len} tokens"
        )
        start_time = time.time()
        dataset = filter_data_by_prompt_length(
            dataset, config.data.max_prompt_len, tokenizer
        )
        logger.success(
            f"Filtered long prompts in {time.time() - start_time:.2f}s - {len(dataset)} samples remaining"
        )
    ...
    if config.data.difficulty_filtering:
        ...
        dataset = dataset.filter(
            ...
            and x[config.data.difficulty_filtering.solve_rate_field]
            <= config.data.difficulty_filtering.max_solve_rate
        )
    ...
    if max_batch_size == "auto":
        ...
        max_batch_size = all_reduce(
            node,
            torch.tensor(local_max_batch_size),
            config=config.parallel.pp,
            op=torch.min,
        ).item()
        logger.info(f"Auto-computed maximum batch size: {max_batch_size}")
        max_batch_size = int(max_batch_size * config.scale_factor)
        logger.info(
            f"Scaled maximum batch size by {config.scale_factor} to {max_batch_size}"
        )
    ...
    if config.sampling.n > max_batch_size:
        raise ValueError(
            f"Sampling.n ({config.sampling.n}) must be less than or equal to max_batch_size ({max_batch_size})"
        )
    ...
    if config.step_path is not None and config.step_path.exists():
        try:
            ...
            logger.info(
                f"Found existing step file at {config.step_path} with step {saved_step}"
            )
            step = saved_step
    ...
    while True:
        if config.rl and config.rl.step_endpoint is not None:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to get step from endpoint {config.rl.step_endpoint}: {e}"
                )
                time.sleep(10)
                continue
        ...
        if config.step_path is not None:
            logger.info(
                f"Writing current inference step ({step}) to {config.step_path}"
            )
        ...
        if node_address_int is not None:
            ...
            generator = np.random.default_rng(
                node_address_int * current_step_batch_counter + step
            )
            ...
            indices = [
                (dataset_offset + j) % len(dataset) for j in range(problems_per_batch)
            ]
            if seed is not None:
                sampling_params.seed = (
                    seed + step * 1_000_000
                )  # 1M is needed to avoid collision from sampling.n

        logger.debug(
            f"Sampling batch with indices [{' '.join(map(str, indices[:3]))}...{' '.join(map(str, indices[-3:]))}]"
        )
        problems = dataset.select(indices)

        verification_infos = [
            json.loads(item["verification_info"]) for item in problems
        ]
        task_types = [item["task_type"] for item in problems]
        prompts = [item["prompt"] for item in problems]

        target_lengths = generate_target_lengths(
            config.rewards.len_reward, len(prompts)
        )
        ...
        if config.contexts:
            ...
            unordered_token_prompts: dict[int, TokensPrompt] = {
                i: TokensPrompt(prompt_token_ids=prompt_token_ids)
                for i, prompt_token_ids in enumerate(tokenized_prompts)
            }
            # Save chunked request outputs (prompt_id -> RequestOutput)
            unordered_output_token_ids: dict[int, list[int]] = {
                i: [] for i in unordered_token_prompts.keys()
            }
            unordered_proofs: dict[int, bytes] = {
                i: b"" for i in unordered_token_prompts.keys()
            }

            max_model_len = llm.llm_engine.model_config.max_model_len
            assert max(config.contexts) <= max_model_len, (
                "The final context should be less than the max model length"
            )
            ...
            for context in config.contexts:
                ...
                local_max_batch_size = compute_max_batch_size(
                    llm, max_model_len=context
                )
                if node:
                    max_batch_size = int(
                        all_reduce(
                ...
                logger.info(
                    f"Auto-computed maximum batch size for context {context}: {max_batch_size}"
                )

                # Micro-batch the sequences to not trigger cache eviction
                prompt_ids = list(unordered_token_prompts.keys())
                micro_batch_prompt_ids = [
                    prompt_ids[i : i + max_batch_size]
                    for i in range(0, len(prompt_ids), max_batch_size)
                ]
                micro_batches = [
                    {
                        prompt_id: unordered_token_prompts[prompt_id]
                        for prompt_id in micro_batch_prompt_ids
                    }
                ...
                for mb_i, mb_unordered_tokens_prompts in enumerate(micro_batches):
                    ...
                    input_token_count = sum(
                        len(token_prompt["prompt_token_ids"])
                        for token_prompt in mb_unordered_tokens_prompts.values()
                    )
                    cacheable_area = max_batch_size * context
                    max_tokens = (cacheable_area - input_token_count) // len(
                        mb_unordered_tokens_prompts
                    )
                    ...
                    assert max_tokens > 0, (
                        "Context must be larger than the max input tokens"
                    )
                    ...
                    logger.info(
                        f"Token prompt IDs: {list(mb_unordered_tokens_prompts.keys())}"
                    )
                    start_time = time.time()
                    request_outputs = llm.generate(
                        list(mb_unordered_tokens_prompts.values()),
                        sampling_params,
                        use_tqdm=config.use_tqdm,
                    )
                    generation_time = time.time() - start_time
                    logger.info(
                        f"Generated {len(request_outputs)} samples in {generation_time:.2f}s"
                    )

                    # Force generation of proofs for the remaining sequences that haven't reached max_len.
                    toploc_cache.maybe_generate_proofs_in_background(
                        force_generate=True
                    )
                    ...
                    chunked_proofs = [
                        b"".join(proofs)
                        for _, proofs in sorted(
                            toploc_cache.proofs.items(), key=lambda x: x[0]
                        )
                    ]
                    for prompt_id, chunked_proof in zip(
                        mb_unordered_tokens_prompts.keys(), chunked_proofs
                    ):
                    ...
                    finished_prompt_ids = []
                    for prompt_id, request_output in zip(
                        mb_unordered_tokens_prompts.keys(), request_outputs
                    ):
                        ...
                        if (
                            ...
                            or finish_sequence
                            or len(request_output.prompt_token_ids)
                    ...
                    logger.info(
                        f"Finished {len(finished_prompt_ids)} sequences at micro batch {mb_i} at context {context}"
                    )
                logger.info(
                    f"Finished {num_finished_sequences} sequences at context {context}"
                )
                        ...
                        outputs=[
                            CompletionOutput(
                                ...
                                text=tokenizer.decode(
                                    unordered_output_token_ids[prompt_id],
            ...
            proofs = list(
                dict(sorted(unordered_proofs.items(), key=lambda x: x[0])).values()
            )
            assert len(request_outputs) == batch_size, (
                "Number of request outputs must match batch size"
            )
            ...
            request_outputs = llm.generate(
                token_prompts, sampling_params, use_tqdm=config.use_tqdm
            )
            ...
            proofs = [
                b"".join(proofs)
                for _, proofs in sorted(toploc_cache.proofs.items(), key=lambda x: x[0])
            ]
            toploc_cache.reset_cache()
        ...
        batch_output_tokens = sum(
            sum(len(output.token_ids) for output in req.outputs)
        ...
        logger.success(
            f"Generated {batch_samples} samples for {batch_problems} problems in {generation_time:.2f}s"
        )
        ...
        logger.debug(
            f"Showing example (first completion):\n{first_prompt}{first_completion}"
        )
        ...
        request_rewards = compute_vllm_rewards(
            request_outputs, verification_infos, task_types, config.rewards
        )
        batch_reward = (
            sum(sum(r.reward for r in req.rewards) for req in request_rewards)
            / batch_samples
        )
        ...
        if sampling_params.seed is not None:
            sampling_seeds = [
                sampling_params.seed + i for i in range(sampling_params.n)
            ] * problems_per_batch
        else:
        ...
        flop_counts = [
            get_inference_input_output_flops(
                config.model.name, len(input_tokens), len(output_tokens)
            )
            for input_tokens, output_tokens in zip(
                table.column("input_tokens").to_pylist(),
                table.column("output_tokens").to_pylist(),
            )
        ]

        work_submission = {
            "output/output_flops": sum(output_flops for _, output_flops in flop_counts)
            // config.parallel.pp.world_size,
    ...
    logger.success(
        f"Inference finished! Generated {total_samples} samples for {total_problems} problems"
    )
...
def main():
    ...
    if inference_envs.SHARDCAST_SERVERS is not None:
        assert config.rl is not None, (
            "RL config is required when SHARDCAST_SERVERS is set"
        )
    ...
    try:
        ...
        if config.parallel.dp > 1:
            if config.parallel.tp == "auto":
                assert torch.cuda.device_count() % config.parallel.dp == 0, (
                    "Number of GPUs must be divisible by DP"
                )
                config.parallel.tp = torch.cuda.device_count() // config.parallel.dp
            gpu_ids = envs.CUDA_VISIBLE_DEVICES
            gpu_ids_per_rank = [
                gpu_ids[i : i + config.parallel.tp]
                for i in range(0, len(gpu_ids), config.parallel.tp)
            ]
            for rank, gpu_ids in enumerate(gpu_ids_per_rank):
                ...
                process = mp.Process(target=EnvWrapper(infer, env), args=(config,))
